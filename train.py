...
from dataloader import CGNSdata
...
def parse_args():
  ...
  parser.add_argument("--ch", type=int, default=32,
                      help="channel multiplier (default: 32)")
...
def main(args):
  ...
  # data loader
  train_dataset = CGNSdata(
      path=args.root,
      train=True,)

  test_dataset = CGNSdata(
      path=args.root,
      train=False,)
  ...
  g_model = Generator(dsp=args.dsp,
                      dspe=args.dspe,
                      ch=args.ch)
  ...
  if args.gan_loss != "none":
    d_model = Discriminator(dsp=args.dsp,
                            dspe=args.dspe,
                            ch=args.ch)
  ...
  for epoch in tqdm(range(args.start_epoch, args.epochs)):
    ...
    for i, sample in enumerate(train_loader):
      sparams = sample[0].to(device)
      image = sample[1].to(device)
      g_optimizer.zero_grad()
      fake_image = g_model(sparams)
      ...
      if args.gan_loss != "none":
        ...
        decision = d_model(sparams, image)
        ...
        fake_decision = d_model(sparams, fake_image.detach())
        ...
        # loss of generator
        g_optimizer.zero_grad()
        fake_decision = d_model(sparams, fake_image)
      ...
      for i, sample in enumerate(test_loader):
        ...
        if i == 0:
          ...
          comparison = torch.cat(
              [image[:n], fake_image.view(len(sparams), 1, 228, 989)[:n]])
